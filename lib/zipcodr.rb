require "sqlite3"
require File.dirname(__FILE__) + "/zipcodr/zipcode"

module Zipcodr

  def self.db
    db = self.open_db
    raise "No database available." unless db.kind_of?(SQLite3::Database)
    db
  end

  def self.open_db
    db = File.dirname(__FILE__) + '/zipcodr/zipcodr.db'
